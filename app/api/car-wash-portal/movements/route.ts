...
export async function GET(request: NextRequest) {
  ...
  const { searchParams } = new URL(request.url); const vehicleId = searchParams.get("vehicleId"); const page = Math.max(1, Number(searchParams.get("page") ?? 1));
  const where = { companyId: context.companyId, ...(context.assignedVehicleId ? { vehicleId: context.assignedVehicleId } : {}), ...(vehicleId ? { vehicleId } : {}) };
  const [operations, total] = await Promise.all([prisma.carWashDailyOperation.findMany({ where, include: { vehicle: { select: { code: true, nameAr: true, nameEn: true } }, location: { select: { nameAr: true, nameEn: true } }, revenues: true, expenses: true }, orderBy: { date: "desc" }, skip: (page - 1) * 20, take: 20 }), prisma.carWashDailyOperation.count({ where })]);
  return NextResponse.json({ success: true, data: operations, page, total, pageSize: 20 });
}
...
export async function POST(request: NextRequest) {
  ...
  const data = parsed.data; const operationDate = day(data.date);
  const vehicle = await prisma.carWashVehicle.findFirst({ where: { id: data.vehicleId, companyId: context.companyId, isActive: true }, select: { id: true, branchId: true } });
  ...
  if (data.kind === "EXPENSE") {
    ...
    const category = await prisma.expenseCategory.findFirst({ where: { id: data.categoryId, companyId: context.companyId }, select: { id: true } });
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    if (data.kind === "EXPENSE") {
      ...
      operation = await tx.carWashDailyOperation.update({ where: { id: operation.id }, data: { totalExpenses: { increment: data.amount }, netRevenue: { decrement: data.amount } } });
      ...
      return { operationId: operation.id, movementId: expense.id };
    }
    ...
    operation = await tx.carWashDailyOperation.update({ where: { id: operation.id }, data: data.kind === "CASH" ? { totalCash: { increment: data.amount }, netRevenue: { increment: data.amount } } : { totalKnet: { increment: data.amount }, netRevenue: { increment: data.amount } } });
    ...
    return { operationId: operation.id, movementId: revenue.id };
  });
