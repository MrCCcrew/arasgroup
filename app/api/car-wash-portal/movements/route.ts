...
const IMAGE_EXTENSIONS: Record<string, string> = { "image/jpeg": "jpg", "image/jpg": "jpg", "image/png": "png", "image/webp": "webp", "image/gif": "gif" };
...
export async function GET(request: NextRequest) {
  ...
  const pageSize = Math.min(500, Math.max(1, Number(searchParams.get("pageSize") ?? 500)));
...
export async function POST(request: NextRequest) {
  ...
  const body = formData
    ? {
        kind: formData.get("kind"), vehicleId: formData.get("vehicleId"), categoryId: formData.get("categoryId") || undefined,
        amount: Number(formData.get("amount")), date: formData.get("date"), notes: formData.get("notes") || undefined,
  ...
  if (image && (!IMAGE_EXTENSIONS[image.type] || image.size > MAX_IMAGE_SIZE)) {
  ...
  const imageUrl = image ? await uploadToR2(`car-wash-movements/${context.companyId}/${nanoid(14)}.${IMAGE_EXTENSIONS[image.type]}`, Buffer.from(await image.arrayBuffer()), image.type) : undefined;
