  ...
  return <div>
    ...
    <main className="page-container space-y-4">
      ...
      {!fiscalYear || !report ? <div className="section-card text-center text-muted-foreground">{t("لا توجد سنة مالية أو فترة صالحة.", "No fiscal year or valid period is available.")}</div> : <>
        <div className="rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900"><div className="flex gap-2 font-medium"><AlertTriangle size={18} />{t("هذا تقرير حركة نقدية صحيح وليس قائمة تدفقات ثلاثية.", "This is a correct cash-movement report, not a three-part cash-flow statement.")}</div><p className="mt-1">{t("دليل الحسابات لا يحتوي تصنيفًا موثوقًا للأنشطة التشغيلية والاستثمارية والتمويلية؛ لذلك لا يتم التخمين.", "The chart of accounts has no reliable Operating/Investing/Financing classification, so no classification is guessed.")}</p></div>
