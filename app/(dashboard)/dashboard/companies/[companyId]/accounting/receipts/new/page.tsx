...
import { ArrowRight, Banknote, Save } from "lucide-react";
...
export default function NewReceiptPage() {
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    if (!form.creditAccountId) {
      setError(locale === "en" ? "Please select the credit account" : "يرجى اختيار الحساب الدائن");
      return;
    }
  ...
  const otherAccounts = accounts.filter((account) => !["REVENUE", "ASSET"].includes(account.type));

  return (
    <div>
      <Header
        title={locale === "en" ? "Receipt Voucher" : "سند قبض"}
        subtitle={locale === "en" ? "Receive funds and create the journal entry automatically" : "استلام مبلغ وتوليد القيد المحاسبي تلقائياً"}
        companyId={companyId}
      />
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="section-card space-y-4">
            <div className="flex items-center gap-2 rounded-lg border border-green-200 bg-green-50 p-3 text-sm">
              <Banknote size={16} className="shrink-0 text-green-600" />
              <span className="text-green-800">
                {locale === "en"
                  ? `Journal preview: debit ${form.paymentMethod === "CASH" ? "cash" : "bank"} to credit the selected account`
                  : `معاينة القيد: مدين ${form.paymentMethod === "CASH" ? "الصندوق" : "البنك"} إلى دائن الحساب المختار`}
              </span>
            </div>
            ...
            <div>
              <label className="mb-1.5 block text-sm font-medium">
                {locale === "en" ? "Received from" : "المستلم منه"}
              </label>
              <input
                type="text"
                ...
                className="input-field w-full"
                placeholder={locale === "en" ? "Client or party name..." : "اسم العميل أو الجهة..."}
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                className="input-field w-full"
                placeholder={locale === "en" ? "Receipt description..." : "وصف عملية القبض..."}
              />
            </div>

            <div>
              <label className="mb-2 block text-sm font-medium">
                {locale === "en" ? "Payment method" : "طريقة القبض"} <span className="text-red-500">*</span>
              </label>
            ...
            <div>
              <label className="mb-1.5 block text-sm font-medium">
                {locale === "en" ? "Credit account" : "الحساب الدائن"} <span className="text-red-500">*</span>
              </label>
              <select
                value={form.creditAccountId}
                onChange={(event) => update("creditAccountId", event.target.value)}
                className="input-field w-full"
                required
              >
                <option value="">{locale === "en" ? "Select account..." : "اختر الحساب..."}</option>
                {revenueAccounts.length > 0 && (
                  <optgroup label={locale === "en" ? "Revenue" : "إيرادات"}>
                ...
                {assetAccounts.length > 0 && (
                  <optgroup label={locale === "en" ? "Assets / Receivables" : "أصول / ذمم"}>
            ...
            <div>
              <label className="mb-1.5 block text-sm font-medium">
                {locale === "en" ? "Reference / Receipt No." : "المرجع / رقم الإيصال"}
              </label>
              <input
                type="text"
          ...
          <div className="flex items-center gap-3">
            ...
            >
              <Save size={16} />
              {loading ? (locale === "en" ? "Saving..." : "جارٍ الحفظ...") : locale === "en" ? "Create receipt" : "إنشاء سند القبض"}
            </button>
            <Link
              href={`/dashboard/companies/${companyId}/accounting/journal-entries`}
