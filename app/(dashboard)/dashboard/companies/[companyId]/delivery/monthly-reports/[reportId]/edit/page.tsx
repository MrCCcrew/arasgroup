...
export default function EditMonthlyReportPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch(`/api/delivery/monthly-reports/${reportId}`).then((r) => r.json()),
    ]).then(([c, d, rep]) => {
      ...
      if (rep.success) {
        ...
        setMonth(r.month);
        setYear(r.year);
        setReportDate(new Date(r.reportDate).toISOString().slice(0, 10));
        setNotes(r.notes ?? "");
        setLines(
          r.lines.length > 0
            ? r.lines.map((l: Record<string, unknown>) => ({
                driverId: String(l.driverId),
                ordersCount: Number(l.ordersCount),
                ratePerOrder: Number(l.ratePerOrder),
                grossAmount: Number(l.grossAmount),
                walletDeducted: Number(l.walletDeducted),
        ...
        setError(rep.error ?? "تعذّر تحميل التقرير");
      }
      setLoading(false);
    });
  }, [companyId, reportId]);
  ...
  async function save() {
    ...
    const res = await fetch(`/api/delivery/monthly-reports/${reportId}`, {
      method: "PATCH",
    ...
    <div>
      <div className="border-b bg-card px-6 py-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold">تعديل التقرير الشهري</h1>
            <p className="text-sm text-muted-foreground">تعديل بيانات التوصيل الشهرية للسائقين</p>
          </div>
        </div>
      </div>
