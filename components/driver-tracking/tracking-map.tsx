...
export interface TrackingMapMarker extends MapPoint { id: string; label: string; status?: string; detailsHref?: string; }
...
export function TrackingMap({ markers = [], route = [] }: { markers?: TrackingMapMarker[]; route?: MapPoint[] }) {
  ...
  return <MapContainer center={center} zoom={markers.length || route.length ? 13 : 8} className="h-[360px] w-full rounded-lg" scrollWheelZoom={false}>
    ...
    {route.length > 0 && <><Marker position={[route[0].latitude, route[0].longitude]} icon={divIcon({ className: "", html: '<span style="display:block;width:16px;height:16px;border-radius:50%;background:#16a34a;border:3px solid white"></span>', iconSize: [16, 16], iconAnchor: [8, 8] })} /><Marker position={[route[route.length - 1].latitude, route[route.length - 1].longitude]} icon={divIcon({ className: "", html: '<span style="display:block;width:16px;height:16px;border-radius:50%;background:#dc2626;border:3px solid white"></span>', iconSize: [16, 16], iconAnchor: [8, 8] })} /></>}
    {markers.map((marker) => <Marker key={marker.id} position={[marker.latitude, marker.longitude]} icon={markerIcon}><Popup><strong>{marker.label}</strong>{marker.status ? <><br />{marker.status}</> : null}{marker.recordedAt ? <><br />{new Date(marker.recordedAt).toLocaleString()}</> : null}{marker.detailsHref ? <><br /><a href={marker.detailsHref}>View details</a></> : null}</Popup></Marker>)}
  </MapContainer>;
}
